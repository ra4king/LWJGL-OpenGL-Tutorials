...
public class TextureGenerator {
	...
	public static class ForcedConvertFlags {
		public static int FORCE_SRGB_COLORSPACE_FMT = 0x0001; // When possible, will force the format to use the sRGB colorspace. Does not cause erroring for formats that can't be sRGB, unless your GL
																// implementation doesn't support sRGB.
		...
		public static int FORCE_LUMINANCE_FMT = 0x0008; // Red and RG textures will become luminance and luminance/alpha textures in all cases. Exceptions will be thrown if the GL implementation does
														// not support those luminance/alpha formats (ie: is core).
		...
		public static int FORCE_SIGNED_FMT = 0x0040; // Image formats that contain unsigned integers will be uploaded as signed integers. Ignored if the format is not an integer/integral format, or if
														// it isn't BC4 or BC5 compressed.
		public static int FORCE_COLOR_RENDERABLE_FMT = 0x0080; // NOT YET SUPPORTED! Will force the use of formats that are required to be valid render targets. This will add components if necessary,
																// but it will throw if conversion would require fundamentally changing the basic format (from signed to unsigned, compressed textures,
																// etc).
		
		public static int FORCE_ARRAY_TEXTURE = 0x0004; // NOT YET SUPPORTED! The texture will be an array texture even if the depth is not present. Ignored for formats that can't be arrays. Will
														// throw if array textures of that type are not supported (ie: cubemap arrays, 2D arrays for lesser hardware, etc).
		public static int USE_TEXTURE_STORAGE = 0x0100; // If ARB_texture_storage or GL 4.2 is available, then texture storage functions will be used to create the textures. Otherwise regular glTex*
														// functions will be used.
		public static int FORCE_TEXTURE_STORAGE = 0x0200; // If ARB_texture_storage or GL 4.2 is available, then texture storage functions will be used to create the textures. Otherwise, an exception
															// will be thrown.
	...
	public static OpenGLPixelTransferParams getUploadFormatType(ImageFormat imageFormat, int forceConvertBits) {
		...
		upload.type = getOpenGLType(imageFormat, pixelDataType);
	...
	private static int getOpenGLType(ImageFormat imageFormat, PixelDataType pixelDataType) {
	...
	private static int getOpenGLFormat(ImageFormat imageFormat, PixelDataType pixelDataType, int forceConvertBits) {
		...
		if(imageFormat.getComponentOrder() == ComponentOrder.BGRA) {
			...
			boolean isRGChannel = getComponentCount(imageFormat) < 3;
	...
	private static boolean isTypeSigned(PixelDataType pixelDataType) {
		...
		return DdsUtil.isOneOfThese(pixelDataType, signedIntegerFormats);
	}
	...
	private static boolean isTypeIntegral(PixelDataType pixelDataType) {
		...
		return DdsUtil.isOneOfThese(pixelDataType, integralIntegerFormats);
	}
	
	private static int getComponentCount(ImageFormat imageFormat) {
		...
		if(DdsUtil.isOneOfThese(imageFormat.getPixelComponents(), twoCompFormats)) {
			return 2;
		}
		
		if(DdsUtil.isOneOfThese(imageFormat.getPixelComponents(), threeCompFormats)) {
			return 3;
		}
		
		if(DdsUtil.isOneOfThese(imageFormat.getPixelComponents(), fourCompFormats)) {
			return 4;
		}
		...
		switch(pixelDataType) {
			case NORM_UNSIGNED_INTEGER:
				...
				if(isSRGBFormat(imageFormat, forceConvertBits)) {
					...
					if(getComponentCount(imageFormat) == 3) {
						return GL_SRGB8;
					} else {
				...
				switch(imageFormat.getBitDepth()) {
					case PER_COMP_8: {
						...
						int numComponents = getComponentCount(imageFormat);
			...
			case NORM_SIGNED_INTEGER: {
				...
				int numComponents = getComponentCount(imageFormat);
				int compSize = perComponentSize(imageFormat);
	...
	private static boolean isSRGBFormat(ImageFormat imageFormat, int forceConvertBits) {
		...
		if(DdsUtil.isOneOfThese(imageFormat.getPixelComponents(), srgbFormats)) {
			return true;
		}
		...
		if(DdsUtil.isOneOfThese(imageFormat.getPixelDataType(), srgbTypes)) {
		...
		return DdsUtil.isOneOfThese(imageFormat.getPixelComponents(), convertableFormats);
	}
	
	private static int perComponentSize(ImageFormat imageFormat) {
		switch(imageFormat.getBitDepth()) {
			case PER_COMP_8:
				return 8;
			case PER_COMP_16:
	...
	private static void createTexture(int textureName, ImageSet imageSet, int forceConvertBits) {
		...
		switch(getTextureType(imageSet, forceConvertBits)) {
			...
			default:
				throw new IllegalArgumentException("Only GL_TEXTURE_2D supported.");
		}
	}
	...
	private static boolean isDirectStateAccessSupported() {
		return GLContext.getCapabilities().GL_EXT_direct_state_access;
	}
	...
	private static void build2DTexture(int textureName, ImageSet imageSet,
		...
		setupUploadState(imageSet.getFormat());
	...
	private static void setupUploadState(ImageFormat imageFormat) {
	...
	private static void manTexStorageBase(int textureTarget,
		...
		for(int mipmap = 0; mipmap < numMipmaps; mipmap++) {
			Dimensions levelDims = DdsUtil.calcMipmapLevelDimensions(imageDimensions, mipmap);
	...
	private static void texSubImage(int texture, int textureTarget,
			...
			OpenGLPixelTransferParams upload, ByteBuffer imageData) {
		// Zero means bound, so no DSA.
		if(texture == 0) {
			texSubImage(textureTarget, mipmapLevel, internalFormat,
					imageDimensions, upload, imageData);
			return;
		}
